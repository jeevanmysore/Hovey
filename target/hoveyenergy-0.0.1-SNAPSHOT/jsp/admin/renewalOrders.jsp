...
<body class="firefox">
   <%@ include file="header.jsp" %>
	<div id="content">
		...
		<div class="renewals" style="text-align: center;width: 75%;margin: 0px auto; background-image: url(/hoveyenergy/images/box-pattern.png);"
	 class="bg_purple1">
			 ...
			 <div style="float: right;text-align: right;">
			      <c:choose>
			      	<c:when test="${page eq 0 && end ne 1 }">
			      		<li>	 <a href="/hoveyenergy/admin/renewals.do?startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${page+1}"><img src="/hoveyenergy/images/next.png" /></a></li>
			      	</c:when>
			      
			        <c:when test="${page eq end-1 && end ne 1 }">
			              <li>  <a href="/hoveyenergy/admin/renewals.do?startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${page-1}"><img src="/hoveyenergy/images/prev.png" /></a></li>
			        </c:when>
				   		 ...
				   		 <tr>
					         ...
					         <td> ${order.accountNumber}</td>
					          <td> ${order.businessName}</td>
					           ...
					           <td><a href="#"  onclick="window.open('/hoveyenergy/printdeal.do?dealId=${order.transDto.transactionId}', 'printOrder', 'width=1280px,height=800px,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,copyhistory=no,resizable=yes');"> 
	            ...
	            <div style="text-align: center;width: 620px;margin: 40px auto;">									
													<ul id="pagin">
													 <!--  For Previous -->	
														...
														<c:if test="${page ne 0 }">
														<li><a  href="/hoveyenergy${requestScope['javax.servlet.forward.servlet_path']}?${query}&startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=0">First</a></li>
															<li>  <a href="/hoveyenergy${requestScope['javax.servlet.forward.servlet_path']}?${query}&startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${page-1}">Prev</a></li>
														</c:if>
																	...
																	<c:forEach var="i" begin="0" end="${end-1 }">
																		<c:choose>
																			<c:when test="${page eq i }">
																				<li><a class="current" href="/hoveyenergy${requestScope['javax.servlet.forward.servlet_path']}?${query}&startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${i}">${i+1 }</a></li>
																			</c:when>
																			<c:otherwise>
																				<li><a  href="/hoveyenergy${requestScope['javax.servlet.forward.servlet_path']}?${query}&startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${i}">${i+1 }</a></li>
																			</c:otherwise>
																		</c:choose>																	
																	</c:forEach>																
																</c:when>
													 ...
													 <!-- For Next -->
													   ...
													   <c:if test="${page lt end-1 }">
															<li>  <a href="/hoveyenergy${requestScope['javax.servlet.forward.servlet_path']}?${query}&startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${page+1}">Next</a></li>
														</c:if>
														<c:if test="${page ne end-1 }">
													  		<li><a  href="/hoveyenergy${requestScope['javax.servlet.forward.servlet_path']}?${query}&startDate=${startDate}&endDate=${endDate}&supplier=${supplier}&status=${status}&contractType=${contractType}&page=${end-1}">Last</a></li>
														</c:if>
													</ul>					
												</div>
